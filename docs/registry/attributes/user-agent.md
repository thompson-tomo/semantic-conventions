...
**[1] `user_agent.name`:** See [Example](https://www.whatsmyua.info) on how to extract the browser's name from the original user agent string. In cases where multiple browsers are embedded (e.g., Chrome-based browsers), the primary browser name SHOULD be selected. With the complete list of browsers stored in `browser.brands`.

In the case of using a user-agent for non-browser products, such as microservices with multiple names/versions inside the `user_agent.original`, the most significant name SHOULD be selected. In such a scenario it should align with `user_agent.version`.
...
Alternatively, this attribute MAY be derived from the [UA client hints API](https://wicg.github.io/ua-client-hints/#interface) (`navigator.userAgentData.platform`). The legacy `navigator.platform` API SHOULD NOT be used.
